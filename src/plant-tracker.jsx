...
async function saveData(key, value) {
  try {
    ...
    if(e.name === 'QuotaExceededError' && key === 'pt_plants') {
      // Photos too large — save plants without photos as fallback
      try {
        const stripped = value.map(p => ({...p, photos: [], primaryPhoto: null}));
        localStorage.setItem(key, JSON.stringify(stripped));
        console.warn('localStorage quota exceeded — photos not saved. Use JSON export to back up photos.');
      } catch {}
    }
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setRooms(r || SEED_ROOMS);
        setPlants(p || SEED_PLANTS);
  ...
  useEffect(() => {
    if (!loaded) return;
    if (plants) saveData("pt_plants", plants);
  }, [plants, loaded]);
  useEffect(() => {
    if (!loaded) return;
  ...
  function importData() {
    try {
      ...
      setRooms(data.rooms); setPlants(data.plants);
      closeImport();
    ...
    <>
      ...
      {showModal && (
        <PlantModal
          ...
          onDelete={editPlant?()=>{ setPlants(ps=>ps.filter(x=>x.id!==editPlant.id)); setShowModal(false); setEditPlant(null); setDetailPlant(null); }:null}
  ...
  return(
    <>
      ...
      {showModal && <PlantModal plant={editPlant} rooms={rooms}
        ...
        onDelete={()=>{ setPlants(ps=>ps.filter(x=>x.id!==editPlant.id)); setShowModal(false); setEditPlant(null); }}
